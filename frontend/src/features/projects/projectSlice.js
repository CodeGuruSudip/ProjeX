...
export const addMember = createAsyncThunk(
  ...
  async (memberData, thunkAPI) => {
    try {
      const token = thunkAPI.getState().auth.user.token;
      return await projectService.addMember(memberData, token);
    } catch (error) {
      const message =
        (error.response &&
          error.response.data &&
...
export const removeMember = createAsyncThunk(
  ...
  async (memberData, thunkAPI) => {
    try {
      const token = thunkAPI.getState().auth.user.token;
      return await projectService.removeMember(memberData, token);
    } catch (error) {
      const message =
        (error.response &&
          error.response.data &&
...
export const updateMemberRole = createAsyncThunk(
  ...
  async (memberData, thunkAPI) => {
    try {
      const token = thunkAPI.getState().auth.user.token;
      return await projectService.updateMemberRole(memberData, token);
    } catch (error) {
      const message =
        (error.response &&
          error.response.data &&
    ...
    builder
      ...
      .addCase(addMember.fulfilled, (state, action) => {
        const project = state.projects.find(
          (p) => p._id === action.meta.arg.projectId
        );
        if (project) {
